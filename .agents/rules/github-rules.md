...
```text
feat: implement BullMQ job scheduler for commerce sync

- added BullMQ queue config
- integrated Upstash Redis connection
- implemented Shopify webhooks worker
- added repeatable job for inventory sync
...
```text
refactor: simplify AutoShipp Intelligence scoring pipeline

- extracted crawler logic
- reduced analysis complexity
- improved Shopify API rate limiting
```
...
```text
feat: add AutoShipp Intelligence Scorer
fix: prevent cross tenant data access
docs: update architecture notes for AES-043
test: add BullMQ worker test coverage
security: enforce Razorpay webhook validation
```
...
```text
feat: implement Intelligence Scorer
```

Bad:

```text
feat: implement Intelligence Scorer, Shopify connector, dashboard API, Razorpay billing
```

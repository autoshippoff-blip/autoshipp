...
- **Why:** A recommendation represents what the engine _told the user at that exact microsecond_. If you update it later because the ML model changed, you corrupt the historical accuracy metric.

## 3. Append-Only Auditing

- **Rule:** `fit_governance_audit_events` and `fit_governance_incidents` MUST NEVER be deleted or updated.
- **Why:** These are compliance records. If an operator makes a mistake, they must issue a _new_ overriding incident, rather than deleting the old one.

...
- Confusing `identity_users` with `customers_customers`. `identity_users` are the _merchants_ logging into the AutoShipp dashboard. `customers_customers` are the _shoppers_ buying clothes on the Shopify storefront.

...
- **Why it matters:** Recommendations are mapped to _Products_, but inventory and purchases are tracked at the _Variant_ level.
...
- **What it is:** A backtesting run. We take 100,000 historical orders and run them through a _new_ ML model version to see if it would have reduced return rates.
...
- **`metadata` (JSONB)**: Used for storing non-indexable, highly volatile data synced from third-party systems (like raw Shopify tags). _Do not use `metadata` for fields you need to query or filter on._

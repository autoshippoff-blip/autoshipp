...
import { Briefcase, Award, Zap, Code, Heart, CheckCircle2, Sparkles, Send, Upload, User, Mail, Phone, Laptop } from 'lucide-react';
...
export default function CareersPage() {
  ...
  const perks = [
    { icon: <Zap className="w-6 h-6 text-brand-orange" />, title: 'AI-Native Scale', desc: 'Work directly on high-throughput LLM voice pipelines handling millions of Indian D2C orders.' },
    { icon: <Laptop className="w-6 h-6 text-brand-blue" />, title: 'High Autonomy', desc: 'Remote-friendly culture with rapid shipping cycles and zero bureaucratic gatekeeping.' },
    { icon: <Award className="w-6 h-6 text-success" />, title: 'Competitive Equity', desc: 'Generous ESOP allocation for early team members shaping unit economics in Indian D2C.' },
  ];
      ...
      <div className="min-h-screen bg-background text-foreground font-sans selection:bg-brand-blue/20 selection:text-foreground relative overflow-hidden">
        ...
        <main className="pt-36 pb-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center max-w-3xl mx-auto mb-20">
            ...
            >
              Build the Intelligence Behind <span className="bg-gradient-to-r from-brand-blue to-teal-500 bg-clip-text text-transparent">Ecommerce</span>
            </motion.h1>
            <motion.p 
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              We are looking for passionate engineers, AI researchers, and growth operators to solve the toughest logistical challenges in India.
            </motion.p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8 mb-20">
            {perks.map((perk, i) => (
              <div key={i} className="bg-card border border-border p-6 sm:p-8 rounded-3xl sm:rounded-[2rem] shadow-xs hover:border-brand-blue/40 transition-all duration-300">
          ...
          <div id="apply" className="max-w-4xl mx-auto bg-card border border-border rounded-3xl sm:rounded-[3rem] p-6 sm:p-16 shadow-2xl relative overflow-hidden">
            ...
            <div className="mb-10 p-6 rounded-3xl bg-amber-500/10 border border-amber-500/20 text-center max-w-2xl mx-auto">
              ...
              <h3 className="text-xl font-bold text-foreground mb-1">We Currently Have No Active Openings</h3>
              <p className="text-sm text-muted-foreground leading-relaxed">
                Our team is currently at full capacity. However, we are always on the lookout for exceptional talent in Chennai. Submit your skills and details below to join our priority talent network for upcoming positions!
              </p>
            </div>

            <div className="text-center mb-12">
              <h2 className="text-3xl sm:text-4xl font-extrabold text-foreground mb-3">Join Our Talent Network</h2>
              <p className="text-muted-foreground">Tell us about your background, skills, and what excites you about Autoshipp.</p>
            </div>
            ...
            {submitted ? (
              ...
              >
                ...
                <p className="text-muted-foreground leading-relaxed text-base">
                  Thank you for applying to Autoshipp. Our engineering talent team will review your skills and portfolio, and reach out for an introductory call if there's a strong fit.
                </p>
                <button 
                  onClick={() => { setSubmitted(false); setFormData({ name: '', email: '', phone: '', role: 'AI / ML Engineer', experience: '1-3 Years', skills: '', portfolio: '', message: '' }); }}
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Role Applying For *</label>
                    <select 
                      value={formData.role}
                    ...
                    >
                      <option value="AI / ML Engineer">AI / ML Engineer (Voice & LLM)</option>
                      <option value="Full Stack Engineer">Full Stack Engineer (Next.js / Node)</option>
                      <option value="Product Manager">Product Manager (Logistics AI)</option>
                      <option value="Growth Specialist">Growth & Enterprise Sales Manager</option>
                      <option value="Logistics Operations">Merchant Success & Operations</option>
                    </select>
                  </div>
                </div>
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Total Experience *</label>
                    <select 
                      value={formData.experience}
                      onChange={(e) => setFormData({...formData, experience: e.target.value})}
                      className="w-full h-12 px-4 rounded-xl bg-background border border-border focus:border-brand-blue focus:outline-none text-sm transition-colors"
                    >
                      <option value="Fresher / Intern">0 - 1 Years (Fresher / Intern)</option>
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                      <Code className="w-3.5 h-3.5" /> LinkedIn / Github / Portfolio
                    </label>
                    <input 
                      type="url" 
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Why Autoshipp? & Note on Experience *</label>
                  <textarea 
                    rows={4}
                    required

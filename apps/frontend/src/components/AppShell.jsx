...
import { LayoutDashboard, Settings, Package, Users, FileText, Phone, Webhook, CreditCard, LogOut, Menu, X } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background flex flex-col md:flex-row">
      ...
      <div className="md:hidden flex items-center justify-between p-4 border-b border-border bg-card text-foreground">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-lg tracking-tight">Autoshipp</span>
        </div>
      ...
      <div className={cn(
        ...
        <div className="h-16 flex items-center px-6 border-b border-sidebar-border text-sidebar-foreground">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-semibold text-xl tracking-tight">Autoshipp</span>
          </div>
          {userRole && <span className="ml-3 px-2 py-0.5 text-xs font-medium bg-sidebar-accent text-sidebar-accent-foreground rounded-full uppercase tracking-wider">{userRole}</span>}
        </div>
          ...
          <nav className="px-3 space-y-1">
            {navigation.map((item) => {
              ...
              return (
                ...
                >
                  <item.icon className={cn("mr-3 flex-shrink-0 h-5 w-5", isActive ? "text-sidebar-accent-foreground" : "text-sidebar-foreground/50")} />
                  {item.name}
                </Link>
              );
            })}
